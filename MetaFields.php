...
abstract class MetaFields {
    ...
    public static function Html(WP_Post $post): void
    {
        ...
        self::Style();
        $saved_user_types = self::GetMetaValue($post->ID, 'banneron_target_users');
        ...
        foreach($possible_values as $val) {
            ...
            if(in_array($val["name"], $saved_user_types)) echo " selected";
            echo ">";
            echo $val['label'];
            echo "</option>";
        }
     ...
     * @return array
     */
    public static function GetMetaValue(int $post_id, string $key): array
    {
	...
	public static function Save(Int $post_id ): Mixed
    {
		...
		if(is_array($_POST['banneron_target_users'])) {
            foreach($_POST['banneron_target_users'] as $input) {
                $target[] = sanitize_text_field($input);
            }
        } else {
